...
cvar_t  *viewmodel_fov;         // Viewmodel FOV (54-120, default: 90)
cvar_t  *viewmodel_offset_x;    // Horizontal offset, + = right (limit: -3.0 to +3.0)
cvar_t  *viewmodel_offset_y;    // Forward/back offset, + = forward (limit: -3.0 to +3.0)
cvar_t  *viewmodel_offset_z;    // Vertical offset, + = up (limit: -3.0 to +3.0)
...
// V_ApplyViewmodelOffset
void V_ApplyViewmodelOffset( struct ref_params_s *pparams, cl_entity_t *view )
{
        if( !view )
                return;

        // This prevents null pointer dereference during early client initialization
        if( !viewmodel_offset_x || !viewmodel_offset_y || !viewmodel_offset_z || !viewmodel_fov )
                return;

        vec3_t forward, right, up;
        AngleVectors( pparams->viewangles, forward, right, up );

        const float SCALE_FACTOR = 10.0f / 3.0f;
        
        // Apply viewmodel position offset with clamped limits (-3.0 to +3.0)
        float clamped_x = max( -3.0f, min( 3.0f, viewmodel_offset_x->value ) );
        float clamped_y = max( -3.0f, min( 3.0f, viewmodel_offset_y->value ) );
        float clamped_z = max( -3.0f, min( 3.0f, viewmodel_offset_z->value ) );
        
        float off_x = clamped_x * SCALE_FACTOR;  // Right/Left (scaled)
        float off_y = clamped_y * SCALE_FACTOR;  // Forward/Back (scaled)
        float off_z = clamped_z * SCALE_FACTOR;  // Up/Down (scaled)

        // Apply offset relative to player's view direction
        view->origin[0] += right[0] * off_x + forward[0] * off_y + up[0] * off_z;
        view->origin[1] += right[1] * off_x + forward[1] * off_y + up[1] * off_z;
        view->origin[2] += right[2] * off_x + forward[2] * off_y + up[2] * off_z;

        // Clamp FOV to valid range: 54-120 degrees
        float baseFov = 90.0f;
        float targetFov = max( 50.0f, min( 120.0f, viewmodel_fov->value ) );

        if( targetFov != baseFov )
        {
                // Calculate scale factor based on FOV difference
                float fovScale = tan( ( baseFov * 0.5f ) * ( M_PI / 180.0f ) ) /
                                 tan( ( targetFov * 0.5f ) * ( M_PI / 180.0f ) );

                // Move weapon forward/back to simulate FOV change
                float fovOffset = ( 1.0f - fovScale ) * 10.0f;
                view->origin[0] += forward[0] * fovOffset;
                view->origin[1] += forward[1] * fovOffset;
                view->origin[2] += forward[2] * fovOffset;
        }
}
...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
        ...
        // ============================================================================
        // Apply custom viewmodel offset and FOV scaling
        // File: cl_dll/view.cpp, Function: V_CalcNormalRefdef()
        // Called after V_CalcQuakeGuns and V_CalcViewModelLag to apply user offsets
        // ============================================================================
        V_ApplyViewmodelOffset( pparams, view );
...
void V_Init (void)
{
        ...
        viewmodel_fov       = gEngfuncs.pfnRegisterVariable( "viewmodel_fov", "90", FCVAR_ARCHIVE );       // FOV range: 54-120
        viewmodel_offset_x  = gEngfuncs.pfnRegisterVariable( "viewmodel_offset_x", "0", FCVAR_ARCHIVE );   // Right/left offset
        viewmodel_offset_y  = gEngfuncs.pfnRegisterVariable( "viewmodel_offset_y", "0", FCVAR_ARCHIVE );   // Forward/back offset
        viewmodel_offset_z  = gEngfuncs.pfnRegisterVariable( "viewmodel_offset_z", "0", FCVAR_ARCHIVE );   // Up/down offset
